...
# 5k full node, cybersecurity lab
...
if [ "$1" == "gui" ] ; then
	if [ "$pruned" == "true" ] ; then
		...
		echo
		src/qt/bitcoin-qt -prune=550 -datadir="$dir" $params -debug=merkleverification
	else
		echo
		src/qt/bitcoin-qt -datadir="$dir" $params -debug=merkleverification
	fi
else
	...
	if [ "$pruned" == "true" ] ; then
		echo "Pruned mode activated, only keeping 550 block transactions"
		echo
		src/bitcoind -prune=550 -datadir="$dir" $params -debug=merkleverification
	else
		echo
		src/bitcoind -datadir="$dir" $params -debug=merkleverification
		# Reindexing the chainstate:
		#src/bitcoind -datadir="/media/sf_Bitcoin" -debug=merkleverification -reindex-chainstate
	fi
fi
